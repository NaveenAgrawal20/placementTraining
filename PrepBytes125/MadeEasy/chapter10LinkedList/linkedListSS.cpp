...
SllNode *nthNode(SllNode *head,int n)
{
    SllNode *pnthNode = head, *pthNode = head;
    for(int i = 1;i<n;i++)
    {
        if(pthNode)
            pthNode = pthNode->next;
    }
    while(pthNode != nullptr)
    {
...
// reverse pair
SllNode *reversePair(SllNode *head)
{
...
int main() {
    ...
    display(ll1->head);
    cout<<nthNode(ll1->head,1)->data<<endl;
    cout<<getMid(ll1->head)->data<<endl;
    ll1->head = reversePair(ll1->head);
    display(ll1->head);
